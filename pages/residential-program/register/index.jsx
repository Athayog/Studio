...
import { registerForResidential, sendToAthayogResidential } from '@/lib/db/forms';
...
function RegisterYoga() {
    ...
    const onSubmit = async (data) => {
        ...
        registerAndSend(data)
            .then(finalResult => {
                ...
                reset()
                setTimeout(() => {
                    window.location.href = 'https://rzp.io/l/C4sU10c'
                }, 1000);
    ...
    async function registerAndSend(data) {
        try {
            ...
            const result1 = await registerForResidential(data);

            // Assuming registerForAcademy resolves with some result needed for sendToAthayog
            const result2 = await sendToAthayogResidential(data);
