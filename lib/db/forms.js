...
export async function registerForAcademy(
     ...
     let scope = 'academyForm'
     if (type == "workshop") {
          scope = "academyWorkshop"
     }

     const storeLinks = firestore.collection(scope).doc();
...
export const sendToAthayog = async (dataFromRegisterForAcademy, type) => {
     try {
          // Assuming dataFromRegisterForAcademy contains necessary information from registerForAcademy
          let titleForm = "Registration For Yoga Academy"
          if (type == "workshop") {
               titleForm = "Registration For Yoga Academy Workshop"
          }

          
          const response = await fetch('https://formsubmit.co/ajax/info@athayogliving.com', {
               method: 'POST',
               headers: {
